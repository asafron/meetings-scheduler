...
type Meeting struct {
	...
	UserPreferredSchoolDay string        `json:"user_preferred_school_day" bson:"user_preferred_school_day"`
...
func GetMeetingDateAsString (day, month, year, startTime, endTime int) string {
	...
	return fmt.Sprintf("%s/%s/%s %s:%s - %s:%s", dayStr, monthStr, yearStr, startHour, startMin, endHour, endMin)
}